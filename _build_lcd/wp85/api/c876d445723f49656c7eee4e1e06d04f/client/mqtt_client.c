...
)
{
    ...
    LE_FATAL_IF(clientThreadPtr==NULL,
                "mqtt_ConnectService() not called for current thread");
...
static void InitCommonData(void)
{
    ...
    _ClientDataPool = le_mem_CreatePool("mqtt_ClientData", sizeof(_ClientData_t));

    // Allocate the client thread pool
    _ClientThreadDataPool = le_mem_CreatePool("mqtt_ClientThreadData", sizeof(_ClientThreadData_t));
    ...
    _HandlerRefMap = le_ref_CreateMap("mqtt_ClientHandlers", 5);
}
...
)
{
    ...
    if ( strlen(brokerUrl) > 256 ) LE_FATAL("strlen(brokerUrl) > 256");


    // Create a new message object and get the message buffer
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_mqtt_Config;
    _msgBufPtr = _msgPtr->buffer;

    // Pack the input parameters
    _msgBufPtr = PackString( _msgBufPtr, brokerUrl );
    _msgBufPtr = PackData( _msgBufPtr, &portNumber, sizeof(int32_t) );
    _msgBufPtr = PackData( _msgBufPtr, &keepAlive, sizeof(int32_t) );
    _msgBufPtr = PackData( _msgBufPtr, &QoS, sizeof(int32_t) );
...
)
{
    ...
    // Range check values, if appropriate
    if ( strlen(password) > 32 ) LE_FATAL("strlen(password) > 32");


    // Create a new message object and get the message buffer
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_mqtt_Connect;
    _msgBufPtr = _msgPtr->buffer;
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_mqtt_Disconnect;
    _msgBufPtr = _msgPtr->buffer;
...
)
{
    ...
    // Range check values, if appropriate
    if ( strlen(key) > 128 ) LE_FATAL("strlen(key) > 128");
    if ( strlen(value) > 128 ) LE_FATAL("strlen(value) > 128");


    // Create a new message object and get the message buffer
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_mqtt_Send;
    _msgBufPtr = _msgPtr->buffer;
    ...
    // Unpack any "out" parameters
    _msgBufPtr = UnpackData( _msgBufPtr, errCodePtr, sizeof(int32_t) );
...
)
{
    ...
    // Range check values, if appropriate
    if ( strlen(json) > 2048 ) LE_FATAL("strlen(json) > 2048");


    // Create a new message object and get the message buffer
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_mqtt_SendJson;
    _msgBufPtr = _msgPtr->buffer;

    // Pack the input parameters
    _msgBufPtr = PackString( _msgBufPtr, json );
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_mqtt_GetConnectionState;
    _msgBufPtr = _msgPtr->buffer;
...
static void _Handle_mqtt_AddSessionStateHandler
(
    void* _reportPtr,
    void* _dataPtr
)
{
    ...
    mqtt_SessionStateHandlerFunc_t _handlerRef_mqtt_AddSessionStateHandler = (mqtt_SessionStateHandlerFunc_t)_clientDataPtr->handlerPtr;
    ...
    _msgBufPtr = UnpackData( _msgBufPtr, &connectErrorCode, sizeof(int32_t) );

    int32_t subErrorCode;
    _msgBufPtr = UnpackData( _msgBufPtr, &subErrorCode, sizeof(int32_t) );


    // Call the registered handler
    if ( _handlerRef_mqtt_AddSessionStateHandler != NULL )
    {
        _handlerRef_mqtt_AddSessionStateHandler( isConnected, connectErrorCode, subErrorCode, contextPtr );
    }
    else
    {
...
)
{
    ...
    mqtt_SessionStateHandlerRef_t _result;
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_mqtt_AddSessionStateHandler;
    _msgBufPtr = _msgPtr->buffer;
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_mqtt_RemoveSessionStateHandler;
    _msgBufPtr = _msgPtr->buffer;
    ...
    _UNLOCK
    addHandlerRef = (mqtt_SessionStateHandlerRef_t)clientDataPtr->handlerRef;
    le_mem_Release(clientDataPtr);
    _msgBufPtr = PackData( _msgBufPtr, &addHandlerRef, sizeof(mqtt_SessionStateHandlerRef_t) );
...
// This function parses the message buffer received from the server, and then calls the user
// registered handler, which is stored in a client data object.
static void _Handle_mqtt_AddIncomingMessageHandler
(
    void* _reportPtr,
    void* _dataPtr
)
{
    ...
    // Pull out additional data from the client data pointer
    mqtt_IncomingMessageHandlerFunc_t _handlerRef_mqtt_AddIncomingMessageHandler = (mqtt_IncomingMessageHandlerFunc_t)_clientDataPtr->handlerPtr;
    void* contextPtr = _clientDataPtr->contextPtr;

    // Unpack the remaining parameters
    char topicName[129];
    _msgBufPtr = UnpackString( _msgBufPtr, topicName, 129 );

    char key[129];
    _msgBufPtr = UnpackString( _msgBufPtr, key, 129 );

    char value[129];
    _msgBufPtr = UnpackString( _msgBufPtr, value, 129 );

    char timestamp[17];
    _msgBufPtr = UnpackString( _msgBufPtr, timestamp, 17 );


    // Call the registered handler
    if ( _handlerRef_mqtt_AddIncomingMessageHandler != NULL )
    {
        _handlerRef_mqtt_AddIncomingMessageHandler( topicName, key, value, timestamp, contextPtr );
    }
    else
    {
        LE_FATAL("Error in client data: no registered handler");
...
)
{
    ...
    mqtt_IncomingMessageHandlerRef_t _result;
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_mqtt_AddIncomingMessageHandler;
    _msgBufPtr = _msgPtr->buffer;
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_mqtt_RemoveIncomingMessageHandler;
    _msgBufPtr = _msgPtr->buffer;
    ...
    le_ref_DeleteRef(_HandlerRefMap, addHandlerRef);
    _UNLOCK
    addHandlerRef = (mqtt_IncomingMessageHandlerRef_t)clientDataPtr->handlerRef;
    le_mem_Release(clientDataPtr);
    _msgBufPtr = PackData( _msgBufPtr, &addHandlerRef, sizeof(mqtt_IncomingMessageHandlerRef_t) );
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_mqtt_AddSessionStateHandler :
            le_event_QueueFunctionToThread(callersThreadRef, _Handle_mqtt_AddSessionStateHandler, msgRef, clientDataPtr);
            break;

        case _MSGID_mqtt_AddIncomingMessageHandler :
            le_event_QueueFunctionToThread(callersThreadRef, _Handle_mqtt_AddIncomingMessageHandler, msgRef, clientDataPtr);
            break;
